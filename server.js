...
// Store admin chat IDs in memory
let adminChatIds = [];
let bot = null;
...
// Jobs - FIXED: Always return an array
async function getJobs() {
  const data = await jsonbinGet();
  if (data?.jobs) {
    // If jobs is an array, return it
    if (Array.isArray(data.jobs)) {
      return data.jobs;
    }
    // If jobs is an object with a jobs property that's an array
    if (data.jobs.jobs && Array.isArray(data.jobs.jobs)) {
      return data.jobs.jobs;
    }
    // If jobs is an object but not array, convert to array
    return [data.jobs];
  }
  return [];
}
...
async function findUserByTelegramChatId(chatId) {
  const users = await getUsers();
  return users.find(u => u.telegramChatId === chatId?.toString());
}
...
async function sendJobAlertsToAllUsers() {
  try {
    ...
    activeJobs.forEach((job, index) => {
      ...
      jobsMessage += `   🏢 ${job.company || 'Company'} - ${job.location || 'UK'}\n`;
      jobsMessage += `   💷 ${job.salary || 'Competitive'}\n`;
      jobsMessage += `   ⏰ ${job.type || 'Full Time'} | ${job.payment_type || 'yearly'} payment\n\n`;
    });
...
async function notifyNewJob(job) {
  try {
    const message = `🎉 *NEW JOB ADDED!*\n\n` +
      ...
      `⏰ ${job.type || 'Full Time'} | ${job.payment_type || 'yearly'} payment\n\n` +
  ...
  try {
    ...
    bot.onText(/\/start/, async (msg) => {
      ...
      const welcomeMessage = 
        ...
        `🔍 *Today's Featured Jobs:*\n${jobsPreview || 'No jobs available at the moment'}\n\n` +
      ...
      if (emailRegex.test(text)) {
        ...
        if (!user) {
          ...
          const welcomeMessage = 
            ...
            `🎯 *Recommended Jobs:*\n${jobsList || 'No jobs available at the moment'}\n\n` +
    ...
    bot.onText(/\/jobs/, async (msg) => {
      ...
      if (activeJobs.length === 0) {
        ...
        activeJobs.forEach((job, i) => {
          ...
          message += `   🏢 ${job.company || 'Company'} - ${job.location || 'UK'}\n`;
          message += `   💷 ${job.salary || 'Competitive'}\n\n`;
        });
      }
    ...
    bot.onText(/\/users/, async (msg) => {
      ...
      users.slice(0, 10).forEach((u, i) => {
        ...
        response += `   📧 ${u.email || 'No email'}\n`;
        response += `   🕐 Last seen: ${u.lastSeen ? new Date(u.lastSeen).toLocaleString() : 'Never'}\n`;
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`📦 JSONBin: Connected (${JSONBIN_BIN_ID})`);
  console.log('='.repeat(60) + '\n');
});
